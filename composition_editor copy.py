...
# Optional audio libs for MP3 and playback
try:
...
class CompositionApp:
    ...
    def __init__(self, root):
        ...
        self.audio_data = None  # raw audio data for playback
        self.audio_sample_rate = 44100
        self.audio_play_obj = None  # simpleaudio playback object
        ...
        try:
            ...
            if self.audio_waveform is not None and len(self.audio_waveform) > 0:
                ...
                if len(samples) > actual_points:
                    # Group samples and get min/max for each group
                    samples_per_group = len(samples) // actual_points
                    trimmed_len = actual_points * samples_per_group
                    trimmed_samples = samples[:trimmed_len]
                    reshaped = trimmed_samples.reshape(actual_points, samples_per_group)
                ...
                # Calculate x positions using the same pixels_per_second as the timeline
                # Each display point represents (audio_duration / actual_points) seconds
                # So x position for point i is: i * (audio_duration / actual_points) * pixels_per_second
                # Which equals: i * audio_duration * (width / total_seconds) / actual_points
                pixels_per_display_point = (self.audio_duration * pixels_per_second) / actual_points if actual_points > 0 else 1
                
                # Create filled polygon using min/max for each point
                poly = []
                for i in range(len(min_vals)):
                    x = i * pixels_per_display_point
                    y_min = mid_y + max_vals[i] * amp_h  # max is bottom
                    y_max = mid_y + min_vals[i] * amp_h  # min is top
                    if i == 0:
                        poly.extend([x, y_min])
                    poly.extend([x, y_min])
                
                # Add right edge and mirror back
                for i in range(len(max_vals)-1, -1, -1):
                    x = i * pixels_per_display_point
                    y_max = mid_y + min_vals[i] * amp_h
                    poly.extend([x, y_max])
                
                try:
                    c.create_polygon(poly, fill="#4caf50", outline="#2e7d32")
                except Exception:
                    # fallback: draw simple line graph
                    try:
                        for i in range(len(max_vals)-1):
                            c.create_line(float(i), mid_y + max_vals[i] * amp_h, 
                                         float(i+1), mid_y + max_vals[i+1] * amp_h, fill="#4caf50")
                    except Exception:
                        pass
    ...
    def add_audio(self):
        ...
        # Don't show messagebox before thread - it blocks the UI!
        thread = threading.Thread(target=self._load_audio_thread, args=(f,), daemon=True)
        thread.start()

    def _load_audio_thread(self, filepath):
        """Load audio file in a background thread to avoid UI freeze."""
        raw_data = None
        wf = None
        sr = 44100
        channels = 1
        ...
        try:
            ...
            # Try loading based on file type and available libraries
            if ext == ".wav":
                raw_data, wf, sr, channels = self._load_wav_file(filepath)
            elif ext == ".mp3":
                if PydubAvailable:
                    raw_data, wf, sr, channels = self._load_audio_with_pydub(filepath)
                else:
                    raise Exception("MP3 format requires pydub installed")
            elif ext in (".flac", ".ogg"):
                if LibrosaAvailable:
                    raw_data, wf, sr, channels = self._load_audio_with_librosa(filepath)
                elif PydubAvailable:
                    raw_data, wf, sr, channels = self._load_audio_with_pydub(filepath)
                else:
                    raise Exception(f"Format {ext} requires librosa or pydub installed")
            else:
                raise Exception(f"Unsupported audio format: {ext}")
            
            # Store raw audio data and sample rate for playback
            self.audio_data = raw_data
            self.audio_sample_rate = sr
            self.audio_channels = channels
            
            # normalize to -1..1 for waveform display
            try:
                ...
                # for stereo, average channels for display
                ...
                print(f"[ERROR] Failed to normalize waveform: {e}")
                import traceback
                traceback.print_exc()
                raise
            ...
            def safe_redraw():
                try:
                    ...
                    import traceback
                    traceback.print_exc()
            self.root.after(0, safe_redraw)
            self.root.after(10, lambda: print("[DEBUG] Audio loading complete"))
            
        except Exception as e:
            print(f"[ERROR] Audio loading failed: {type(e).__name__}: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _load_wav_file(self, path):
        """Load WAV file and return (raw_audio_data, waveform_for_display, sample_rate, channels)."""
        with wave.open(path, "rb") as wf:
            ...
            if nchan > 1:
                ...
                # For waveform display, average channels
                waveform = arr.mean(axis=1).astype(np.float32)
            else:
                waveform = arr.astype(np.float32)
            
            # Return raw audio data for playback, and waveform for display
            return arr, waveform, sr, nchan

    def _load_audio_with_librosa(self, path):
        """Load audio file with librosa (optimized for fast loading and display).
        Returns (raw_audio_data, waveform_for_display, sample_rate, channels)."""
        if not LibrosaAvailable:
            raise Exception("librosa not available")
        # Load mono, float32 for efficiency
        signal, sr = librosa.load(path, sr=None, mono=True, dtype=np.float32)
        channels = 1
        
        # Return signal as raw_data, signal as waveform (already normalized by librosa)
        return signal, signal, sr, channels

    def _load_audio_with_pydub(self, path):
        """Load audio file with pydub and return (raw_audio_data, waveform_for_display, sample_rate, channels)."""
        ...
        if seg.channels > 1:
            ...
            # For waveform display, average channels
            waveform = samples.mean(axis=1).astype(np.float32)
        else:
            waveform = samples.astype(np.float32)
        
        # Return raw audio data for playback, and waveform for display
        return samples, waveform, sr, seg.channels
        ...
        try:
            if self.is_playing:
                ...
                try:
                    ...
                    print(f"[ERROR] Failed to configure play button: {e}")
                
                try:
                    pygame.mixer.music.stop()
                except Exception as e:
                    print(f"[ERROR] Failed to stop audio: {e}")
                ...
                if self.playback_time > 0.0:
                    try:
                        ...
                        print(f"[ERROR] Failed to configure stop button: {e}")
        
                else:
                    try:
                        self.stop_btn.forget()
                    except Exception as e:
                        print(f"[ERROR] Failed to hide stop button: {e}")

                if self.playback_job:
                    try:
                        self.root.after_cancel(self.playback_job)
                    except Exception as e:
                        print(f"[ERROR] Failed to cancel playback job: {e}")
                    finally:
                        self.playback_job = None
            else:
                ...
                try:
                    pygame.mixer.music.play(start=self.playback_time)
                except Exception as e:
                    pass
                # start loop
                self.playback_loop()
    ...
    def start_audio_playback(self):
        """Start or resume audio playback from the current playback_time."""
        if not SimpleaudioAvailable or self.audio_data is None:
            return
        ...
        try:
            ...
            # Calculate the starting sample based on current playback time
            start_sample = int(self.playback_time * self.audio_sample_rate)
            
            # Get the audio data from the starting point
            audio_data = self.audio_data.copy()
            if start_sample > 0 and start_sample < len(audio_data):
                audio_data = audio_data[start_sample:]
            
            # Ensure audio data is int16 for simpleaudio
            if audio_data.dtype != np.int16:
                # Normalize and convert to int16
                max_val = float(np.max(np.abs(audio_data)))
                if max_val <= 0:
                    max_val = 1.0
                audio_data = np.clip(audio_data / max_val * 32767, -32768, 32767).astype(np.int16)
            
            # CRITICAL: Ensure audio_data is C-contiguous in memory (required by simpleaudio)
            if not audio_data.flags['C_CONTIGUOUS']:
                audio_data = np.ascontiguousarray(audio_data)
            
            self.audio_play_obj = sa.play_buffer(audio_data, self.audio_channels, 2, self.audio_sample_rate)
        except Exception as e:
            import traceback
            traceback.print_exc()
    ...
    def playback_loop(self):
        try:
            ...
            try:
                ...
                print(f"[ERROR] perf_counter error: {e}")
            ...
            if self.playback_time > self.audio_duration and self.audio_duration > 0:
                ...
                try:
                    ...
                    print(f"[ERROR] Failed to stop audio in loop: {e}")
            ...
            try:
                ...
                print(f"[ERROR] Frame count error: {e}")
                self._playback_frame_count = 0
            ...
            try:
                ...
                print(f"[ERROR] draw_playback_cursor failed: {e}")
                try:
                    self.redraw_timeline()
                except Exception as e2:
                    print(f"[ERROR] redraw_timeline failed: {e2}")

            # Occasionally do a full render/redraw (to keep visuals updated)
            # NOTE: Only redraw render view (for animations), NOT timeline
            # (redrawing timeline causes stuttering in playback cursor movement)
            try:
                if self._playback_frame_count % 8 == 0:
                    # Update render view for animations, but NOT timeline
                    # Timeline cursor is kept smooth by draw_playback_cursor() only
                    self.redraw_render()
            except Exception as e:
                print(f"[ERROR] redraw_render failed: {e}")
            ...
            if self.is_playing:
                try:
                    ...
                    print(f"[ERROR] Failed to schedule next playback_loop: {e}")
            else:
                pass
        except Exception as e:
            print(f"[ERROR] playback_loop crashed: {type(e).__name__}: {e}")
            import traceback
            traceback.print_exc()
            self.is_playing = False
            try:
    ...
    def draw_playback_cursor(self):
        ...
        try:
            ...
            # Ensure cursor is on top of timeline items (without update_idletasks for better performance)
            try:
